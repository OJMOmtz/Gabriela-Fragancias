        ...
        try:
            ...
            for i, file_path in enumerate(self.files_selected):
                ...
                try:
                    table = dbf.Table(file_path)
                    table.open()
                    df = pd.DataFrame(list(table))  # Convertir a DataFrame
                    table.close()
                except Exception as e:
                ...
                # Validar si las columnas seleccionadas existen en el archivo DBF
                dbf_columns = df.columns
                for pg_col, combo in file_mapping.items():
                    dbf_col = combo.get()
                    if dbf_col and dbf_col not in dbf_columns:
                        messagebox.showerror(
                            "Error",
                            f"La columna '{dbf_col}' no existe en el archivo {file_name}. Verifica el mapeo."
                        )
                        return
                    ...
                    for pg_col, combo in file_mapping.items():
                        ...
                        if not dbf_col:
                            ...
                            if pg_col == 'fecha_nacimiento' or pg_col == 'fecha_defuncion':
                                record.append(self.transform_date(value))
                            elif pg_col == 'sexo':
                                record.append(self.transform_sex(value))
                            elif isinstance(value, str):
                                record.append(self.clean_text(value))
                            else:
                                record.append(value)
                    transformed_data.append(tuple(record))

                # Cargar datos a PostgreSQL
                try:
                    placeholders = ', '.join(['%s'] * len(self.pg_columns))
                    query = f"INSERT INTO tu_tabla_postgresql ({', '.join(self.pg_columns)}) VALUES ({placeholders})"
                    cursor.executemany(query, transformed_data)
                    conn.commit()
                    records_processed += len(transformed_data)
                except Exception as e:
                    messagebox.showerror("Error", f"Error al cargar datos de {file_name}: {e}")
                    conn.rollback()
                    continue
                ...
                self.detail_label['text'] = (
                    f"Detalles: {records_processed} registros procesados, "
                    f"tiempo restante: {int(remaining_time)} segundos"
                )
