    ...
    try:
        ...
        for idx, (archivo_csv, estructura) in enumerate(archivos_csv.items(), 1):
            ...
            try:
                ...
                start_time = time()
                total_lotes = (len(registros_validos) + batch_size - 1) // batch_size
                
                with tqdm(total=len(registros_validos), 
                    ...
                    for i in range(0, len(registros_validos), batch_size):
                        batch = registros_validos.iloc[i:i + batch_size]
                ...
                registros_por_segundo = len(registros_validos) / tiempo_total
                print(f"\nRegistros procesados: {len(registros_validos):,}")
