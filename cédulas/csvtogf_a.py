...
def cargar_archivos_csv_a_postgresql(archivos_csv, tabla_destino):
    ...
    try:
        ...
        for archivo_csv, estructura in archivos_csv.items():
            ...
            # Preparar log de errores
            log_errores = []

            # Insertar datos fila por fila con manejo de errores
            start_time = time()
            with tqdm(total=len(df), desc=f"Cargando {os.path.basename(archivo_csv)}", unit="registro") as pbar:
                for _, row in df.iterrows():
                    try:
                        ...
                        log_errores.append((row.to_dict(), str(e)))
                    pbar.update(1)

            # Guardar log de errores si existen
            if log_errores:
                log_df = pd.DataFrame(log_errores, columns=['Fila', 'Error'])
                log_file = f"log_errores_{os.path.basename(archivo_csv)}.csv"
