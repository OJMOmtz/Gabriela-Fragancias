...
def insertar_datos_postgresql(data, table_name, progress_bar, progress_label):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            progress_label['text'] = "Insertando datos en PostgreSQL..."

            # Inserta cada fila
            for i, record in enumerate(data):
                cursor.execute(query, list(record.values()))
                progress_bar['value'] = i + 1
                progress_label['text'] = f"Insertando datos en PostgreSQL... ({i+1}/{len(data)})"
...
def main():
    ...
    def ejecutar_importacion():
        ...
        insertar_datos_postgresql(data, table_name, progress_bar, progress_label)
