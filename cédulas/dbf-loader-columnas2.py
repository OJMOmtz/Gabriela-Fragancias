...
class DBFLoader:
    ...
    def process_record(self, record: Dict) -> Optional[tuple]:
        try:
            ...
            sexo = self.get_value_with_alternatives(record, ['SEXO', 'CODIGO_SEX'], '').strip().upper()[:1]
            direccion = self.clean_address(self.get_value_with_alternatives(record, ['DIRECCION', 'DIRECC'], None))

            id_distrito = self.get_value_with_alternatives(record, ['COD_DIST', 'DISTRITO'], '').strip()
            id_dpto = self.get_value_with_alternatives(record, ['COD_DPTO', 'DEPART'], '').strip()
            zona = self.get_value_with_alternatives(record, ['ZONA'], '').strip()
            lugar_nacimiento = self.get_value_with_alternatives(record, ['LUGAR_NACIMIENTO', 'LUG_NAC', 'LUGNAC'], '').strip()
            fecha_defuncion = self.standardize_date(self.get_value_with_alternatives(record, ['FECHA_DEFUNCION', 'FEC_DEFUNC', 'FEC_DEF'], None))
