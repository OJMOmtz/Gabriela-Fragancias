    ...
    def read_csv(self, file_path: str):
        ...
        for encoding in encodings_to_try:
            try:
                ...
                self.logger.info(f"Archivo CSV leído con encoding: {encoding}")
                return data.to_dict(orient='records')
            except Exception as e:
                self.logger.warning(f"Intento fallido para leer {file_path} con encoding {encoding}: {e}")
        self.logger.error(f"Error leyendo CSV {file_path}: No se pudo determinar el encoding adecuado.")
        return []
