...
class CedulasETL:
    ...
    def read_csv_with_error_handling(self, archivo, encoding):
        ...
        try:
            # Intentar leer el archivo CSV con un manejo flexible de codificación
            df = pd.read_csv(archivo, dtype=str, encoding=encoding, on_bad_lines='skip', errors='replace')
            return df
        except Exception as e:
            logging.error(f"Error leyendo el archivo {archivo}: {e}")
            return None

    def read_csv_with_flexible_encoding(self, archivo):
        """Leer CSV con una codificación más flexible si es necesario."""
        try:
            # Intentar leer el archivo con codificación más permisiva
            encoding = "latin1"  # Cambiar a una codificación más flexible si es necesario
            df = pd.read_csv(archivo, dtype=str, encoding=encoding)
            return df
        except Exception as e:
            logging.error(f"Error leyendo el archivo {archivo} con codificación {encoding}: {e}")
            return None
        ...
        try:
            ...
            for archivo in self.archivos_fuente:
                try:
                    ...
                    if df is None:
                        # Si no se pudo leer el archivo con el encoding detectado, intentar con codificación flexible
                        df = self.read_csv_with_flexible_encoding(archivo)
                    if df is not None:
