...
def usar_copy_para_insercion(cursor, tabla_destino, columnas, registros):
    """Utiliza COPY para insertar registros en PostgreSQL de forma eficiente."""
    from io import StringIO
    buffer = StringIO()
    registros.to_csv(buffer, index=False, header=False)
    buffer.seek(0)
    cursor.copy_expert(f"""
        COPY {tabla_destino} ({', '.join(columnas)})
        FROM STDIN WITH CSV
    """, buffer)
    ...
    try:
        ...
        for idx, (archivo_csv, estructura) in enumerate(archivos_csv.items(), 1):
            ...
            try:
                ...
                usar_copy_para_insercion(cursor, tabla_destino, columnas_a_insertar, registros_validos)
                conn.commit()
