    ...
    def read_csv(self, file_path: str):
        ...
        for encoding in encodings_to_try:
            for delimiter in delimiters_to_try:
                try:
                    data = pd.read_csv(file_path, sep=delimiter, engine='python', encoding=encoding, on_bad_lines='skip')
                    self.logger.info(f"Archivo CSV leído con encoding: {encoding} y delimitador: '{delimiter}'")
                    return data.to_dict(orient='records')
                except Exception as e:
                    self.logger.warning(f"Intento fallido para leer {file_path} con encoding {encoding} y delimitador '{delimiter}': {e}")
        self.logger.error(f"Error leyendo CSV {file_path}: No se pudo determinar el encoding o delimitador adecuado.")
        return []
