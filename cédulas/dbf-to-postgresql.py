...
class DBFLoader:
    ...
    def process_record(self, record: Dict) -> tuple:
        ...
        cedula = record.get('cedula', 'numero_ced').strip()
        nombre = record.get('nombre', '').strip()
        apellido = record.get('apellido', '').strip()
        sexo = record.get('sexo', 'codigo_sex').strip()
        fec_nac = self.standardize_date(record.get('fec_nac','fec_naci'))
        direcc = record.get('direcc', 'direccion').strip()
        distrito = record.get('distrito', 'cod_dist').strip()
        depart = record.get('id_dpto', 'cod_dpto').strip()
        zona = record.get('zona', '').strip()
        lug_nac = record.get('lugar_nac', '').strip()
        fec_def = self.standardize_date(record.get('fec_def'))
            ...
            for table_path in self.tables:
                try:
                    ...
                    execute_batch(cur, """
                        ...
                        (numero_cedula, nombre, apellido, sexo, fecha_nacimiento, direccion, id_barrio, id_distrito id_dpto, zona, lugar_nacionamiento, fecha_defuncion)
                        VALUES (%s, %s, %s, %s, %s, %s)
