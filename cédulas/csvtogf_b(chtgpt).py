...
def es_fecha_valida(fecha):
    """Verifica si una fecha es válida, incluyendo años bisiestos."""
    try:
        pd.Timestamp(fecha)
        return True
    except Exception:
        ...
        for idx, (archivo_csv, estructura) in enumerate(archivos_csv.items(), 1):
            ...
            try:
                ...
                if 'fecha_nacimiento' in df.columns:
                    ...
                    df['fecha_valida'] = df['fecha_nacimiento'].apply(lambda x: es_fecha_valida(x) if pd.notnull(x) else False)
