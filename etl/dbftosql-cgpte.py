...
class DBFToPostgres:
    ...
    def process_record(self, record, cursor):
        ...
        try:
            ...
            mapped_record = {}
            for col in self.pg_columns:
                value = record.get(col.lower())
                
                # Limpiar y transformar valores según su tipo
                if isinstance(value, (int, float)):  # Si es un número
                    value = str(value).strip()
                elif isinstance(value, str):  # Si es una cadena
                    value = value.strip()
                elif value is None:  # Si es nulo
                    value = None
                
                # Transformar fechas
                if col in ['fecha_nacimiento', 'fecha_defuncion'] and value:
                    value = self.transform_date(value)
                
                mapped_record[col] = value or self.fetch_from_other_source(col, record.get('cedula'), cursor)

            # Validar que al menos `numero_cedula` no sea nulo
            if not mapped_record['numero_cedula']:
                return None
