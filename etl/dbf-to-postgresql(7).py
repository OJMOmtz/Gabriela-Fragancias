...
class DBFLoader:
    def __init__(self):
        ...
        self.sexo_mapping = {1: 'F', 2: 'M'}  # Mapeo personalizable
    ...
    def process_record(self, record):
        ...
        try:
            ...
            cedula = self.safe_str(record.get('cedula', 'Nroced'))
            cedula = cedula.replace('.', '')  # Eliminar puntos de la cédula
            nombre = self.safe_str(record.get('nombre'))
            apellido = self.safe_str(record.get('apellido', 'Apelli'))
            ...
            fec_nac = None
            for field in ['fec_nac', 'fecha_nac', 'fechanac', 'Fechanac', 'fnac']:
                if field in record:
                ...
                try:
                    ...
                    if file_extension == '.dbf':
                        ...
                        with open(table_path, 'r', encoding='utf-8') as file:                            
                            table = list(csv.DictReader(file, delimiter=';', quoting=csv.QUOTE_NONE))
                            for record in table:
                                for key in record:
                                    record[key.lower()] = record.pop(key)
                    else:
                        self.logger.warning(f"Tipo de archivo no soportado: {table_path}")
                        continue
                    
                    self.logger.info(f"Procesando archivo: {table_path}")
                    self.logger.info(f"Campos disponibles: {table[0].keys()}")
    ...
    def select_files(self):
        """Permite al usuario seleccionar los archivos DBF, CSV o TXT"""
        file_paths = filedialog.askopenfilenames(filetypes=[("Archivos DBF", "*.dbf"), ("Archivos CSV", "*.csv"), ("Archivos TXT", "*.txt")])
        if file_paths:
            self.tables.clear()
